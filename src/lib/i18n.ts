...
const en = {
  ...
  readyForNext: 'Done with this tab?',
  saveAndStartNew: 'Save & start new',
  saveAndStartNewHelp: 'Keep this plan in History, then clear the workspace.',
  savedAndStartedNew: '“{title}” saved. New settlement ready.',
...
const zhCN: MessageTable = {
  ...
  historyMore: '{title} + 另外 {count} 笔', settlementFallback: '结算 · {date}', onThisDevice: '保存在此设备', savedSettlements: '已保存的结算', closeHistory: '关闭历史记录', saveSettlement: '保存当前结算', saveSettlementHelp: '保存本地快照，之后可随时重新打开。', settlementName: '结算名称', historyPlaceholder: '周末旅行、合租账单…', saveLocally: '保存到本机', readyForNext: '这笔账算完了？', saveAndStartNew: '保存并新建', saveAndStartNewHelp: '存入历史记录，然后清空当前账单。', savedAndStartedNew: '已保存“{title}”，可以开始新账单了。', addExpenseBeforeSave: '请先添加一笔支出再保存。', savedCounter: '已保存 {count} / 50', noSavedTitle: '还没有已保存的结算', noSavedBody: '保存后的方案会留在此设备并显示在这里。', deleteLabel: '删除{title}', peopleCount: '{count} 人', openPlan: '打开方案', historyPrivacy: '这里的数据不会上传或同步。',
...
const es: MessageTable = {
  ...
  historyMore: '{title} + {count} más', settlementFallback: 'Plan · {date}', onThisDevice: 'En este dispositivo', savedSettlements: 'Planes guardados', closeHistory: 'Cerrar historial', saveSettlement: 'Guardar este plan', saveSettlementHelp: 'Guarda una copia local para abrirla más tarde.', settlementName: 'Nombre del plan', historyPlaceholder: 'Viaje de fin de semana, facturas del piso…', saveLocally: 'Guardar localmente', readyForNext: '¿Has terminado este plan?', saveAndStartNew: 'Guardar y empezar otro', saveAndStartNewHelp: 'Guárdalo en el historial y limpia el espacio de trabajo.', savedAndStartedNew: '“{title}” guardado. Ya puedes empezar otro plan.', addExpenseBeforeSave: 'Añade un gasto antes de guardar este plan.', savedCounter: '{count} / 50 guardados', noSavedTitle: 'Aún no hay planes guardados', noSavedBody: 'Los planes guardados permanecen en este dispositivo y aparecerán aquí.', deleteLabel: 'Eliminar {title}', peopleCount: '{count} personas', openPlan: 'Abrir plan', historyPrivacy: 'Nada de esto se sube ni se sincroniza.',
