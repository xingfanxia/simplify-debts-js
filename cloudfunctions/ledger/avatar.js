// Generated from miniprogram/lib/avatar.js. Do not edit directly.
const AVATAR_EMOJIS = Object.freeze([
  '🐶', '🐱', '🐰', '🐻', '🐼', '🐨', '🐯', '🦁', '🐮', '🐷',
  '🐸', '🐵', '🐔', '🐧', '🐦', '🦊', '🐴', '🐝', '🐙', '🐢',
  '🐬', '🦋', '🐹', '🐺', '🦦',
  '🍎', '🍊', '🍋', '🍉', '🍇', '🍓', '🍒', '🍑', '🍍', '🥭',
...
function automaticAvatarEmoji(seed, usedEmojis = []) {
...
function randomAvatarEmoji(usedEmojis = [], random = Math.random) {
  const used = new Set(Array.from(usedEmojis).filter(isAvatarEmoji))
...
function ensureParticipantAvatars(participants, ledgerSeed = 'local-ledger') {
...
function avatarToneClass(seed) {
  return AVATAR_TONES[avatarHash(`tone:${seed}`) % AVATAR_TONES.length]
}

function avatarPresentation(avatarEmoji, seed = '') {
